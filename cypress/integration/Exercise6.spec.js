...
describe('Corp tech contact ', function() {
    ...
    beforeEach('URL Assertion', function(){
        ...
        cy.visit('https://instaprotek.dnaqa.net/portal')
        cy.url().should('include','/login')
        cy.fixture('insta').then(testdata  => {
            cy.wait(2000)
       ...
       cy.wait(5000) 
       cy.url().should('eq', 'https://instaprotek.dnaqa.net/portal/dashboard')
    })
       //check menu
       it('check menu',()=>{
        cy.contains('Timeline').click()
        cy.wait(1000)
        cy.url().should('eq', 'https://instaprotek.dnaqa.net/portal/timeline')
        cy.contains('Users').click()
        cy.wait(1000)
        cy.url().should('eq', 'https://instaprotek.dnaqa.net/portal/user')
        cy.contains('Customers').click()
        cy.wait(1000)
        cy.url().should('eq', 'https://instaprotek.dnaqa.net/portal/customer')
        cy.contains('Calls').click()
        cy.wait(1000)
        cy.url().should('eq', 'https://instaprotek.dnaqa.net/portal/call')
        cy.contains('Registrations').click()
        cy.wait(1000)
        cy.url().should('eq', 'https://instaprotek.dnaqa.net/portal/registration')
        cy.contains('Purchase').click()
        cy.wait(1000)
        cy.url().should('eq', 'https://instaprotek.dnaqa.net/portal/purchase')
        cy.contains('Repair Shops').click()
        cy.wait(1000)
        cy.url().should('eq', 'https://instaprotek.dnaqa.net/portal/shop')
        cy.contains('Affiliate').click()
        cy.wait(1000)
        cy.url().should('eq', 'https://instaprotek.dnaqa.net/portal/affiliate')
        cy.contains('Claim Reports').click()
        cy.wait(1000)
        cy.url().should('eq', 'https://instaprotek.dnaqa.net/portal/claim')
       })
       ...
       it('registration',()=>{
        ...
        cy.get('.md-dialog-footer').contains('Next').click()
         cy.fixture('insta').then(testdata  => {
             ...
             cy.wait(5000)
             cy.get('#snackbar-alert-dialog > p').contains('Customer successfully created')
             cy.get('#barcode').type(testdata.barcode).should('have.value',testdata.barcode)
             cy.get('#validate-product-barcode').click()
             cy.wait(2000)
             // cy.get('#snackbar-alert-dialog > p').contains('Barcode successfuly validated')
             cy.contains('Next').click()
             cy.wait(2000)
             cy.get('#undefined-panel-3 > div > div.md-cell.md-cell--8.md-grid.deviceRegistration-content > div.multiSelect__withIcon.md-cell.md-cell--12.md-cell--12-phone.default-checkbox > div.multiSelect__withIcon__select > div > div > div > span > span').click({force:true})
             
             //cy.contains('Device').click()
            //  cy.get('#react-select-8--value > div.Select-value').click({force:true})
             cy.contains(testdata.deviceDetails.device).click({force:true})
             //cy.get('#undefined-panel-3 > div > div.md-cell.md-cell--4.deviceReg > div > div > img').should('have.attr','src','https://instaprotek-upload.dnaqa.net/file?path=/images/device/2020-07-30T06:32:48.623Z.png')
        ...
        cy.wait(2000)
        cy.contains('Next').click()
        cy.get('#undefined-panel-5 > div > div > div > div > div > div > fieldset:nth-child(1) > div:nth-child(2) > label > div > i').click()
        ...
        cy.fixture('insta').then(testdata  => {
            ...
            //cy.get('[class="summary-row "]').should('contain','Have you purchased your phone in the past 60 days?:')
            //cy.get('[class="summary-row "]').should('contain','Do you currently have insurance on your phone?:')
